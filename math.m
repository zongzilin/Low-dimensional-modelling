classdef math
    methods(Static)
        ...
        function result = diff_phi(phi, diff_weight)
    

            result = diff_weight * real(phi) + 1i * diff_weight * imag(phi);

        end

        function [w_int]= f_chebyshev_int_weight(N)
            ...
            for i = 0 : N-1
                ...
                for ni=0:(N+1)/2-3
                   w_int(i+1) = w_int(i+1) + cos(pi*(2*(ni+1)*i)/(N-1))/((2*(ni+1))^2-1);   
                end
                
                w_int(i+1) = w_int(i+1) + .5*cos(pi*i)/((N-1)^2-1);
                w_int(i+1) = -4./(N-1)*w_int(i+1);
                
                if (i == 0  || i == (N-1))
                    w_int(i+1)=w_int(i+1)*.5;
                end
